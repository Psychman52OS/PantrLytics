...
- `port` (optional): HTTP port the app listens on inside the container (defaults to `8099`). In Home Assistant, set the host port in the Network section; the add-on also keeps port 8099 open for compatibility.

All options can also be supplied as environment variables (`BASE_URL`, `IPP_HOST`, `IPP_PRINTER`, `SERIAL_PREFIX`, `PORT`) when running outside Home Assistant. Data is stored under `/data` in the add-on; set `DATA_DIR` if you want a different location when running locally.
...
export PORT=8099  # change as needed; defaults to 8099
./run.sh  # serves on http://localhost:${PORT}
```
